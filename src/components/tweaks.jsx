// tweaks.jsx — in-app Settings panel + form-control helpers.
// Adapted from the design-tool "Tweaks" scaffold into a real, controlled settings
// panel: useTweaks persists to localStorage; TweaksPanel is opened via open/onClose
// props (a floating gear button in App drives it) instead of the design host protocol.
...
// ── useTweaks ───────────────────────────────────────────────────────────────
// Single source of truth for settings values, persisted to localStorage.
const TWEAKS_KEY = 'cc.tweaks.v1';
export function useTweaks(defaults) {
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    setValues((prev) => {
      ...
      try { localStorage.setItem(TWEAKS_KEY, JSON.stringify(next)); } catch (e) { /* ignore */ }
      return next;
    });
  }, []);
...
// ── TweaksPanel ─────────────────────────────────────────────────────────────
// Controlled floating settings panel. Draggable by its header.
...
// TweakColor — curated color picker (single hex per option).
